...
public class CustomTestNGReportListener extends BaseTestListener
		implements IReporter, WebDriverEventListener, IExecutionListener, ITestListener {
	...
	private String dReportFileName = "/report/automation_report.html";
	...
	protected void writeReportTitle(String title) {
		writer.println("<center><h6 style=\"color: #525252;margin-bottom: 15px; margin-top: 15px;\">" + title + " - "
				+ getCurrentDateTime() + "</h6></center>");
	}
	...
	protected void writeHead() {
		...
		writer.println("<title>Automation Report</title>");
	...
	protected void writeStylesheet() {
		writer.println(
				"<link rel=\"stylesheet\" type=\"text/css\" href=\"http://netdna.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css\" />");
		writer.println(
		...
		writer.print(
				".table tr.header { font-weight: bold; background-color: #fff; cursor: pointer; -webkit-user-select: none; /* Chrome all / Safari all */ -moz-user-select: none; /* Firefox all */ -ms-user-select: none; /* IE 10+ */ user-select: none; /* Likely future */ }");
	...
	protected void writeBody() {
		...
		writer.println(
				"<script> $(document).ready(function() {  var ua = navigator.userAgent,  event =(ua.match(/iPad/i)) ? \"touchstart\" : \"click\"; if ($('.table').length > 0){$('.table .header').on(event, function() {$(this).toggleClass(\"active\",\"\").nextUntil('.header').css('display',function(i, v) {return this.style.display === 'table-row' ? 'none' : 'table-row'; }); }); }});</script>");
		writer.println("</body>");
	}
	...
	protected void writeSuiteSummary() {
		...
		writer.println(
				"<table style=\"border: 3px solid #999;background-color: #fff;\" class=\"table table-bordered overall-table\">");
	...
	protected void writeScenarioSummary() {
		...
		writer.print(
				"<thead class=\"thead-dark\"><tr class=\"detail-result-header\"><th colspan=\"7\">DETAILED RESULTS</th></tr></thead>");
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					buffer.append("<td><a href=\"#m").append(scenarioIndex).append("\">").append(methodName)
							.append("</a></td>").append("<td class\"se\" rowspan=\"").append("\">")
							.append(shortException).append("</td>").append("<td class=\"fs\" rowspan=\"").append("\">")
							.append(fs).append("</td>").append("</td>").append("<td rowspan=\"").append("\">")
							.append(formatter.format(startTime.getTime())).append("</td>").append("<td rowspan=\"")
							.append("\">").append(formatter.format(endTime.getTime())).append("</td>")
							.append("<td rowspan=\"").append("\">").append(convertTimeToString(dur)).append("</td>")
							.append("</tr>");
					scenarioIndex++;
				...
				writer.print("\">");
				writer.print(Utils.escapeHtml(classResult.getClassName().substring(27)));
				writer.print("</td>");
				writer.print(buffer);
	...
	private int writeScenarioDetails(List<ClassResult> classResults, int startingScenarioIndex) {
		...
		for (ClassResult classResult : classResults) {
			...
			for (MethodResult methodResult : classResult.getMethodResults()) {
				...
				String label = Utils.escapeHtml(results.iterator().next().getMethod().getMethodName());
	...
	private void writeScenario(int scenarioIndex, String label, ITestResult result) {
		writer.print(
				"<div class=\"panel-body\"><table class=\"table table-bordered\"><thead class=\"thead-dark\"><tr><th>EXCEPTION DETAIL</th></tr></thead><tr><td><table style=\"border:3px solid #ffeeba;\" class=\"table table-bordered result\">");
		...
		writer.print("</span>");
		writer.println(
				"<span style=\"float:right;\"><a class=\"btn btn-primary\" role=\"button\" href=\"#summary\">back to summary</a></span>");
		writer.print("</td>");
		writer.print("</tr>");
